...
class l1p_class():
    def __init__(self, T_init, y):#,*args, **kwargs
        ...
        self.dict_sines = None #dictionary whose columns are only sines and cosines
...
#                 dataset_names = 'data',                 
        ...
        start_time_l1p = time.clock()
        if self.dict is None:
            raise Exception('To run the l1 periodogram you must define a dictionary (method self.set_dictionary)')
        
        Nomega = len(self.omegas)
       
#        self.weights = weights
#        if self.V is None:
#            raise Exception('To use the l1 periodogram, the noise covariance matrix must be defined, set the attribute V as self.V = covariance matrix')
#        self.starname = starname
#        self.Nphi = Nphi 
#        
#        # ------- Frequency grid ------- #
#        self.omegas = omegas
#        if self.omegas is None or update_values:
#        #If the frequency grid is not set in input create a frequency
#        #grid with a max frequency and an oversampling value
#            if verbose >=1:
#                print('Creating frequency grid')
#            self.create_freqgrid(omegamax, oversampling)  
        
#        
#        # ------- Covariance ------- #
#        if self.W is None or update_values:
#            if verbose >=1:            
#                print('Creating precision matrix')
#            self.create_precision_matrix()
#
#        # ------ Non constrained vectors   ------- #  
#        if self.offsets is None:
#            self.offsets = np.ones((self.Nt,1))
#        if self.MH0 is None:
#            self.MH0 = self.offsets
                     

        dim_H0 = np.shape(self.MH0)[1]            
           
        # ------ Define the dictionary ------ #
#        if self.dict_sines is None or update_values:
#            self.dict_sines = A_sines        
#        if A is not None or update_values:#self.dict is None:
#            self.dict = A
#       
        ...
        #if verbose >=1:
            #print('in spell: max memory', resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)
...
#            if self.dict is None or update_values:
#                if verbose >=1:
#                    print('Creating dictionary (lars)')
#                start_time = time.clock()
#                #LARS requires centered and normalize columns
#                self.create_dictionary(self.Nphi, center = True, 
#                                       normalize=True,
#                                       verbose=verbose)
#                if verbose >=1:
#                    print('Dictionary creation duration', time.clock() - start_time, "seconds") 
#            else:
#                if verbose >=1:
#                    print('The dictionary is already set')
            # ------- LARS input ------- #            
            ...
            if verbose >=1:
                print('LARS duration', time.clock() - start_time, "seconds")
...
#            #Group lasso requires Nphi = 2
#            self.Nphi = 2
#            if self.dict is None or update_values:
#                if verbose >=1:
#                    print('Creating dictionary (gglasso)')
#                start_time = time.clock()
#                self.create_dictionary(self.Nphi, 
#                                       center = True, 
#                                       orthonormalize=True,
#                                       verbose=verbose)
#                if verbose >=1:
...
#            
            ...
            #lambdas_input = np.array([0.11])/self.Nt
            ...
            dict_out = gglasso_basis_pursuit_l1p.gglasso(y_input, 
                                  ...
                                  **kwargs)
            if verbose >=1:
                print('gglasso duration', time.clock() - start_time, "seconds")
        ...
        if verbose >=1:
            print('Smoothing duration ', time.clock() - start_time, "seconds")
                                      
        # Compute at which frequencies the peaks are attained    
        start_time = time.clock()
        self.find_peaks()   
        if verbose >=1:
            print('Find peaks duration', time.clock() - start_time, "seconds")
        ...
        if plot_output:
            ...
            self.plot_clean(N_markers,
                            ...
                            **kwargs)            
        if verbose >=1:
            print('Plot (clean) duration ', time.clock() - start_time, "seconds")
        ...
        if verbose >=1:
            print('Significance evaluation duration', time.clock() - start_time1, "seconds")
        ...
        if plot_output:
            ...
            self.plot_with_list(N_markers,
                               ...
                               **kwargs) 
            if verbose >=1:           
                print('Plot (with list) duration ', time.clock() - start_time, "seconds")
                
        if verbose>=1:
            print('--------- l1 periodogram: end ----------')
            print('Total time: ', time.clock() - start_time_l1p, ' seconds')
            print('----------------------------------------')
...
# ------------------------------------------------------------------- #
    ...
    def set_dict(self,
        ...
        self.update_dict(omegamax = omegamax,
    ...
    def update_dict(self,
        ...
        if verbose >=1:
            start_time = time.clock()
            print('Creating dictionary, basis pursuit numerical method: ' +  self.numerical_method)
        ...
        if numerical_method == 'gglasso':
            print('using gglasso: setting Nphi to 2')
            if self.Nphi != 2:
                recompute_dict_sine = True
                recompute_dict = True            
            Nphi = 2 
        ...
        if omegamax is not None:
            ...
            if self.omegamax is not None:
                ...
                if omegamax <= self.omegamax and allconst:
                    ...
                    self.dict_sines = self.dict_sines[:,:i*self.Nphi]
                    if verbose >=1:
        ...
        if recompute_dict:
            ...
            if numerical_method=='gglasso':
                #LARS requires orthogonalized columns
                self.center_dict()
                if verbose >=1:
...
#    def create_dictionary(self, Nphi,
#                              normalize = False,
#                               orthonormalize = False,
#                               center = True, 
#                               verbose=1):
#        '''
#        Create a dictionary of sine functions A weighted by matrix W, 
#        and projected on 
#        with centered and normalized columns
#        '''
#        Nomega = len(self.omegas)
#        self.Nphi = Nphi
#        if len(self.omegas)==0:
#            raise Exception('To define a dictionary, there must be at least one frequency in the frequency grid (set attribute omegas)')
#        if self.W is None:
#            raise Exception('To define a dictionary, the precision matrix (attribute W) must be defined')       
#        if self.MH0 is None:
#            raise Exception('To define a dictionary, the null hypothesis matrix (attribute MH0) must be defined')       
#        if self.weights is None:
#            self.weights = np.ones(Nomega*Nphi)
#            
#        if self.dict_sines is None:
#            if verbose >=1:
#                print('Setting dict_sines')
#            self.create_dict_sine(Nphi)
#        else:
#            if verbose >=1: 
#                print('dict_sines is already set')
#        
#        if verbose >=1:
#            print('Setting projection matrix, {} unconstrained vectors'.format(self.MH0.shape[1]))
#        self.create_projmat()
#        if verbose >=1:
#            print('Weighting dictionary')
#        self.weight_dict_sines()
#        
#        if center:
#            if verbose >=1:
#                print('Centering dictionary columns')
#            self.center_dict()
#        if normalize: 
#            self.normalize_dict()
#
#        #print('in spell: before orthonormalize')  
#        #print(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)
#        #objgraph.show_most_common_types()        
#        
#        #self.__delattr__('dict_sines')
#        if orthonormalize:
#            if Nphi == 2:
#                if verbose >=1:
#                    print('Orthonormalizing the vectors in gglasso')
#                self.orthonormalize_2d_dict(verbose=verbose)   
#            else:
#                if verbose >=1:
#                    print('The orthonormalization of the dictionary can be done only if Nphi = 2, orthonormalization not performed' )
#
...
#        sinomegas = np.sin(omT)     
        self.dict_sines = np.zeros((self.Nt,Nomega*Nphi))
...
#                sinom =sinomegas[:,ii]
    ...
    def weight_dict_sines(self):
        ...
        if self.projmat is not None:
            self.dict = self.W.dot(self.projmat.dot(self.dict_sines))
        else:
            self.dict = self.W.dot(self.dict_sines)
    ...
    def orthonormalize_2d_dict(self, verbose=1):
        ...
        if self.Nphi != 2:
            raise Exception('Nphi must be set to two to orthonormalize the dictionary, use c.update_dict')
    ...
    def unpenalize_periods(self,periods,MH0_core,**kwargs):
        ...
        self.update_dict(MH0=M1,**kwargs)
